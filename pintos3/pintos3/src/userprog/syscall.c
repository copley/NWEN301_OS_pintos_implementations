...
/*MY CODE STARTS HERE*/
    ...
    char *kernel_file = copy_in_string(ufile);
    int pid = process_execute(kernel_file);
...
/*MY CODE STARTS HERE*/
    lock_acquire(&fs_lock);
    char *kernel_file = copy_in_string(ufile);
    int success = filesys_create(kernel_file, initial_size);
    palloc_free_page(kernel_file);
    lock_release(&fs_lock);    
    return success;
/*MY CODE ENDS HERE*/
...
/*MY CODE STARTS HERE*/
    lock_acquire(&fs_lock);
    char *kernel_file = copy_in_string(ufile);
    int success = filesys_remove(kernel_file);
    palloc_free_page(kernel_file);
    lock_release(&fs_lock);
...
/*MY CODE STARTS HERE*/
    lock_acquire(&fs_lock);
    char *kernel_file = copy_in_string(ufile);
    struct file *new_file = filesys_open(kernel_file);
    palloc_free_page(kernel_file);
    ...
    new_fd->handle = thread_current()->next_handle++;
...
struct file_descriptor * get_file_by_handle(int handle)
{
    ...
    struct file_descriptor *cur_fds = list_begin(&current->fds);
    while(cur_fds!=list_end(&current->fds)){
        if(cur_fds->handle==handle){
              return cur_fds;
        }
        cur_fds = list_next(cur_fds);
    }
    return NULL;
}
/*MY CODE ENDS HERE*/
...
/*MY CODE STARTS HERE*/
    lock_acquire(&fs_lock);    
    struct file_descriptor *f_d = get_file_by_handle(handle);
    if(f_d==NULL){
        lock_release(&fs_lock);
        sys_exit(0);
    }        
    struct file *file = f_d->file;
    if(file==NULL){
        lock_release(&fs_lock);
        sys_exit(0);
    }  
...
/*MY CODE STARTS HERE*/  
  ...
  {  
     ...
     while(sz>0){
         left_in_page = PGSIZE - pg_ofs(udst);
         if(sz>left_in_page){
             bytes_read = file_read (r_file, udst, left_in_page); 
         }
         else{
             bytes_read = file_read (r_file, udst, sz);
         }
        sz-= bytes_read;
        udst+= bytes_read;
...
{
/*MY CODE STARTS HERE*/
    struct file_descriptor *fd = get_file_by_handle(handle);
    if(fd==NULL){
        lock_release(&fs_lock);
        sys_exit(0);
    }        
