...
process_execute (const char *file_name)
{
  ...
  file_copy = strtok_r(file_copy," ",&p);
  ...
  if(tid!=TID_ERROR){
    ...
    if(exec.success){//2
      list_push_back(&thread_current()->children, &exec.wait_status->elem);//3
    }
    else{
      tid = TID_ERROR;//4
    }
...
/*MY CODE STARTS HERE*/
  if (success){
    exec->wait_status = thread_current()->wait_status = malloc(sizeof *exec->wait_status);//1
...
/*MY CODE STARTS HERE*/
  ...
  while(child_elem!=list_end(&current->children)){//1

    struct wait_status *cws = list_entry(child_elem, struct wait_status, elem);

    if(cws->tid == child_tid){//2
      sema_down(&cws->dead);//3
      int exit_code = cws->exit_code;//4
...
/*MY CODE STARTS HERE*/  
  ...
  if(&cur->wait_status->ref_cnt==0){//2
    free(cur);
  }
  
  struct list_elem *child_elem = list_begin(&cur->children);

  while(child_elem!=list_end(&cur->children)){//3 
    struct wait_status *cws = list_entry(child_elem, struct wait_status, elem);//4
